...
class Test_DataCollector:
    ...
    def compute_events(self, prev_gray, gray):
        ...
        for i in range(len(gray)):
            ...
            pos = events_sc>0
            neg = events_sc<0
            events_vis[pos, 2] = (255 * events_sc[pos]).astype(np.uint8)    # Red channel
            events_vis[neg, 0] = (255 * -events_sc[neg]).astype(np.uint8)   # Blue channel
    ...
    def save_images(self, env, samples_per_height, mode, sample_id):
            ...
            for target_z in heights:
                ...
                while height_sample_count < samples_per_height:
                    ...
                    # Phase 2 — capture event frame and save
                        ...
                        depth_vis = np.clip(depth, 0.0, 1.0) * 255
        ...
        self.log(f"Test {mode} collection complete. Total samples: {sample_id}")
...
def main():
    ...
    data = Test_DataCollector()
    env = data.create_env(1)
    data.create_directories(env.num_envs)

    data.log("\nConnecting to Unity for Expert...........")
    env.connectUnity()
    sample_id = data.save_images(env, 600, "Expert", 0)   # 1111 per (speed, height) segment — total ~9999 10000 // 9
    ...
    time.sleep(20.0)
    subprocess.run(['python3', '/home/kishore/ev_nav/collector/close_range_collector.py', '--mode', 'test', '--sample_id', str(sample_id)], check=True)
