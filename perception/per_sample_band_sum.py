...
class EvalDataset(Dataset):
    ...
    Also builds `self.episodes` (list of consecutive-index lists) from
    episode_boundaries.json so the ConvLSTM hidden state can be rolled out
    per episode and reset exactly at each boundary — mimicking deployment.
    Falls back to one-episode-per-environment if no boundaries file exists.
    """

    def __init__(self, dataset_dir):
        self.samples = []  # (event_path, depth_path, sample_id)
        self._env_end_indices = []
        ...
        # Episode boundaries (last frame of each episode) — same source as dataset.py
        import json
        boundaries_path = os.path.join(dataset_dir, 'episode_boundaries.json')
        if os.path.exists(boundaries_path):
            with open(boundaries_path) as f:
                ends = set(json.load(f).get('episode_end_global_indices', []))
            print(f'[EvalDataset] episode boundaries loaded from {boundaries_path}')
        else:
            ends = set(self._env_end_indices)
            print(f'[EvalDataset] no episode_boundaries.json — using environment '
                  f'boundaries only ({len(ends)} episodes)')
        self.episodes = self._build_episodes(ends)
        print(f'[EvalDataset] {len(self.episodes)} episodes for temporal rollout')

    def _build_episodes(self, episode_end_set):
        """[[idx0, idx1, ...], [idxN, ...], ...] — consecutive frames per episode."""
        episodes, start = [], 0
        for end in sorted(episode_end_set):
...
def main():
    ...
    # One batch == one episode, frames in temporal order. Passing h=None per
    # forward gives a fresh ConvLSTM state at each episode start and a proper
    # temporal rollout within — matching real deployment. (BATCH_SIZE unused.)
    loader  = DataLoader(dataset, batch_sampler=dataset.episodes,
