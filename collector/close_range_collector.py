...
class Val_DataCollector:
    ...
    def __init__(self):
        ...
        self.save_dir_raw = os.path.join(os.environ['PROJECT_PATH'], 'datasets/Val/DEPTH_RAW/')
        self.save_dir_eve_raw = os.path.join(os.environ['PROJECT_PATH'], 'datasets/Val/EVENTS_RAW/')
        #self.save_dir_labels = os.path.join(os.environ['PROJECT_PATH'], 'datasets/Val/LABELS/')

        log_dir = os.path.join(os.environ['PROJECT_PATH'], 'datasets/Val/logs/')
    ...
    def save_images(self, env, samples_per_height, mode, sample_id):
        ...
        self.log(f"Validation Data collection complete. Total samples: {sample_id}")
...
def main():
    ...
    time.sleep(20.0)
    subprocess.run(['python3', '/home/kishore/ev_nav/collector/close_range_collector.py', '--mode', 'val', '--sample_id', str(sample_id)], check=True)
