...
class DataCollector:
    ...
    def create_directories(self, obs):
    ...
    def save_images(self, env):
        ...
        for step in range(1000):
            ...
            if step>0 and step%capture_interval == 0:
                ...
                # after computing events change prev_gray
                prev_gray = gray.copy()
                ...
                for i in range(env.num_envs):
                    cv2.imwrite(os.path.join(os.path.join(self.save_dir_rgb, f'environment_{i+1}'), f'rgb_sample{sample_id:04d}.png'), rgb[i])
                    cv2.imwrite(os.path.join(os.path.join(self.save_dir_dep, f'environment_{i+1}'), f'depth_sample{sample_id:04d}.png'), depth_vis[i].astype(np.uint8))
                    np.save(os.path.join(os.path.join(self.save_dir_raw, f'environment_{i+1}'), f'depth_raw_sample{sample_id:04d}.npy'), depth[i])
                    cv2.imwrite(os.path.join(os.path.join(self.save_dir_eve, f'environment_{i+1}'), f'event_vis_sample_{sample_id:04d}.png'), eve_vis[i])
                    np.save(os.path.join(os.path.join(self.save_dir_eve_raw, f'environment_{i+1}'), f'event_raw_sample{sample_id:04d}.npy'), eve_raw[i])


                sample_id += 1
                print(f"Sample {sample_id} : saved | images saved at {self.save_dir_rgb} ")
...
def main():
    ...
    data = DataCollector()
    env = data.create_env(2)
    
    obs = env.reset()
    if obs.ndim == 1:
        obs = obs.reshape(1, -1)
    data.create_directories(obs)

    print("\nConnecting to Unity...........")
    env.connectUnity()
    data.save_images(env)
