            ...
            if epoch % 5 == 0:
                ...
                with torch.no_grad():
                    for event, depth, _ in val_loader:
                        ...
                        mask = (depth >= 0) & (depth <= DEPTH_THRESH)
                        val_mae_sum  += ((pred[mask] - depth[mask]).abs() * 100.0).sum().item()
                        val_n_pixels += mask.sum().item()
                val_loss /= n_val
                val_mae   = val_mae_sum / val_n_pixels if val_n_pixels > 0 else float('inf')
...
if __name__ == '__main__':
    study = optuna.create_study(
        study_name="unet_depth_optuna-mod-run1",
    ...
    subprocess.run([
        'torchrun', f'--nproc_per_node={train_gpus}', '--master_port=29600',
        '-m', 'perception.train'
    ], env=env)
