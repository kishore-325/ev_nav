...
class OrigUNet(nn.Module):
    ...
    def __init__(self, evs_min_cutoff=1e-3):
        super().__init__()
        self.evs_min_cutoff = 1e-3
        self.input_h = 260
        self.input_w = 346

        #----------Encoder-----------
        self.e11 = nn.Conv2d(2, 32, kernel_size=3, padding=0)    # (N,32,258,344)
    ...
    def _form_input(self, x):
        """
        Split single-channel log-diff into 2-channel (neg, pos).
        x: (N, 1, H, W)
        returns: (N, 2, H, W)  channel-0 = |neg events|, channel-1 = pos events
        """
        x = x.clone()
        ...
        return torch.stack([ch_neg, ch_pos], dim=1)     # (N,2,H,W)
    
    @staticmethod
    def _crop(enc, target_h, target_w):
        """Centre-crop encoder feature map to (target_h, target_w)."""
        h, w = enc.shape[2], enc.shape[3]
        r0 = h//2 - target_h//2
        c0 = w//2 - target_w//2
        return enc[:, :, r0:r0+target_h, c0:c0+target_w]
    
    def forward(self, x):
        """
        x: (N, 1, 260, 346) single-channel log-diff
        returns: (N, 1, 260, 346) depth in [0, 1]
        """
